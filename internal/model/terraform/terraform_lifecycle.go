...
type TerraformPhase struct {
	...
	ReleaseRetentionPolicy             TerraformPolicy `hcl:"release_retention_policy"`
	TentacleRetentionPolicy            TerraformPolicy `hcl:"tentacle_retention_policy"`
}

type TerraformPolicy struct {
	QuantityToKeep    *int    `cty:"quantity_to_keep"`
	ShouldKeepForever *bool   `cty:"should_keep_forever"`
	Unit              *string `cty:"unit"`
}
