...
// 2. The blank space is optionally populated with some shared global resources. This is only done when testing the export of projects that use data source lookups to reference existing resources. An empty string passed as the prepopulate directory disables tis step.
// 3. The source space is populate with the supplied TF files, while the import space is populated with the TF files generated by exporting the source space.
//
