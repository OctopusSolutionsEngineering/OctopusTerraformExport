...
type Arguments struct {
	...
	ExperimentalEnableStepTemplates bool            `json:"experimentalEnableStepTemplates" jsonschema:"Has no effect. This option used to enable the export of step templates, but this is now a standard feature. This option is left in for compatibility."`
	Profiling                       bool            `json:"profiling" jsonschema:"Enable profiling. Run 'pprof -http=:8080 octoterra.prof' to view the results."`
	ExcludeTerraformVariables       bool            `json:"excludeTerraformVariables" jsonschema:"This option means the exported module does not expose Terraform variables for common inputs like the value of project or library variables set variables. This reduces the size of the Terraform configuration files, but makes the module less configurable because values are hard coded."`
	...
	Url                             string          `json:"url" jsonschema:"The Octopus URL e.g. https://myinstance.octopus.app"`
	ApiKey                          string          `json:"apiKey" jsonschema:"The Octopus api key"`
	...
	RunbookId                       string          `json:"runbookId" jsonschema:"Limit the export to a single runbook. Runbooks are exported referencing external resources as data sources."`
	RunbookName                     string          `json:"runbookName" jsonschema:"Limit the export to a single runbook. Requires projectName or projectId. Runbooks are exported referencing external resources as data sources."`
	LookupProjectDependencies       bool            `json:"lookupProjectDependencies" jsonschema:"Use data sources to lookup the external project dependencies. Use this when the destination space has existing environments, accounts, tenants, feeds, git credentials, and library variable sets that this project should reference."`
	LookupProjectLinkTenants        bool            `json:"lookupProjectLinkTenants" jsonschema:"When lookupProjectDependencies is true, lookupProjectLinkTenants will reestablish the link to tenants that were linked to the source project and recreate any project and common tenant variables. Essentially this means the exported project 'owns' the relationship to the tenant and any variables used by the tenant."`
	Stateless                       bool            `json:"stepTemplate" jsonschema:"Create an Octopus step template"`
	StatelessAdditionalParams       StringSliceArgs `json:"stepTemplateAdditionalParameters" jsonschema:"Indicates that a non-secret variable should be exposed as a parameter. The format of this option is 'ProjectName:VariableName'. This option is only used with the -stepTemplate option."`
	StepTemplateName                string          `json:"stepTemplateName" jsonschema:"Step template name. Only used with the stepTemplate option."`
	StepTemplateKey                 string          `json:"stepTemplateKey" jsonschema:"Step template key used when building parameter names. Only used with the stepTemplate option."`
	StepTemplateDescription         string          `json:"stepTemplateDescription" jsonschema:"Step template description used when building parameter names. Only used with the stepTemplate option."`
	IgnoreCacManagedValues          bool            `json:"ignoreCacManagedValues" jsonschema:"Pass this to exclude values managed by Config-as-Code from the exported Terraform. This includes non-sensitive variables, the deployment process, connectivity settings, and other project settings. This has no effect on projects that do not have CaC enabled."`
	ExcludeCaCProjectSettings       bool            `json:"excludeCaCProjectSettings" jsonschema:"Pass this to exclude any Config-As-Code settings in the exported projects. Typically you set -ignoreCacManagedValues=false -excludeCaCProjectSettings=true to essentially 'convert' a CaC project to a regular project. Values from the 'main' or 'master' branches will be used first, or just fall back to the first configured branch."`
	...
	DefaultSecretVariableValues     bool            `json:"defaultSecretVariableValues" jsonschema:"Pass this to set the default value of secret variables to the octostache template referencing the variable."`
	DummySecretVariableValues       bool            `json:"dummySecretVariableValues" jsonschema:"Pass this to set the default value of secret variables, account secrets, feed credentials to a dummy value. This allows resources with secret values to be created without knowing the secrets, while still allowing the secret values to be specified if they are known. This option takes precedence over the defaultSecretVariableValues option."`
	InlineVariableValues            bool            `json:"inlineVariableValues" jsonschema:"Inline the project and library variable set variable values rather than exposing their value as a Terraform variable. Secret variables will be inlined as dummy values. This option takes precedence over DummySecretVariableValues and DefaultSecretVariableValues."`
	ProviderVersion                 string          `json:"providerVersion" jsonschema:"Specifies the Octopus Terraform provider version."`
	ExcludeProvider                 bool            `json:"excludeProvider" jsonschema:"Exclude the provider from the exported Terraform configuration files. This is useful when you want to use a parent module to define the backend, as the parent module must define the provider."`
	IncludeProviderServerDetails    bool            `json:"includeProviderServerDetails" jsonschema:"Define the server URL and API keys as variables passed to the provider. Set this to false to use the OCTOPUS_ACCESS_TOKEN, OCTOPUS_URL, and OCTOPUS_APIKEY environment variables to configure the provider."`
	IncludeOctopusOutputVars        bool            `json:"includeOctopusOutputVars" jsonschema:"Capture the Octopus server URL, API key and Space ID as output variables. This is useful when querying the Terraform state file to locate where the resources were created."`
	...
	LimitResourceCount              int             `json:"limitResourceCount" jsonschema:"For internal use only. Limits the number of resources of a given type that are returned. For example, a value of 30 will ensure the exported Terraform only includes up to 30 accounts, and up to 30 feeds, and up to 30 projects etc. This is used to reduce the output when octoterra is used to generate a context for an LLM. This limit is a guide and it is possible that more than the specified number of resources are returned due to multiple goroutines adding resources to the output."`
	GenerateImportScripts           bool            `json:"generateImportScripts" jsonschema:"Generate Bash and Powershell scripts used to import resources into the Terraform state."`
	IgnoreCacErrors                 bool            `json:"ignoreCacErrors" jsonschema:"Ignores errors that would arise when a project can not resolve configuration in a Git repo."`

	OctopusManagedTerraformVars string `json:"octopusManagedTerraformVars" jsonschema:"Specifies the name of an Octopus variable to be used as a template string in the body of the terraform.tfvars file. This allows Octopus to inject all the variables used by Terraform from a variable containing the contents of a terraform.tfvars file."`

	IgnoreProjectChanges         bool `json:"ignoreProjectChanges" jsonschema:"Use the Terraform lifecycle meta-argument to ignore all changes to the project (including its variables) when exporting a single project."`
	IgnoreProjectVariableChanges bool `json:"ignoreProjectVariableChanges" jsonschema:"Use the Terraform lifecycle meta-argument to ignore all changes to the project's variables when exporting a single project. This differs from the ignoreProjectChanges option by only ignoring changes to variables while reapplying changes to all other project settings."`
	IgnoreProjectGroupChanges    bool `json:"ignoreProjectGroupChanges" jsonschema:"Use the Terraform lifecycle meta-argument to ignore the changes to the project's group."`
	IgnoreProjectNameChanges     bool `json:"ignoreProjectNameChanges" jsonschema:"Use the Terraform lifecycle meta-argument to ignore the changes to the project's name."`
	LookUpDefaultWorkerPools     bool `json:"lookUpDefaultWorkerPools" jsonschema:"Reference the worker pool by name when a step uses the default worker pool. This means exported projects do not inherit the default worker pool when they are applied in a new space."`
	IncludeIds                   bool `json:"includeIds" jsonschema:"For internal use only. Include the 'id' field on generated resources. Note that this is almost always unnecessary and undesirable."`
	IncludeSpaceInPopulation     bool `json:"includeSpaceInPopulation" jsonschema:"For internal use only. Include the space resource in the space population script. Note that this is almost always unnecessary and undesirable, as the space resources are included in the space creation module."`
	IncludeDefaultChannel        bool `json:"includeDefaultChannel" jsonschema:"Internal use only. Includes the 'Default' channel as a standard channel resource rather than a data block."`
	...
	*/
	...
	ExcludeAllProjectVariables       bool            `json:"excludeAllProjectVariables" jsonschema:"Exclude all project variables from being exported. WARNING: steps that used this variable may no longer function correctly."`
	ExcludeProjectVariables          StringSliceArgs `json:"excludeProjectVariable" jsonschema:"Exclude a project variable from being exported. WARNING: steps that used this variable may no longer function correctly."`
	ExcludeProjectVariablesExcept    StringSliceArgs `json:"excludeProjectVariableExcept" jsonschema:"All project variables except those defined with excludeProjectVariableExcept are excluded. WARNING: steps that used other variables may no longer function correctly."`
	ExcludeProjectVariablesRegex     StringSliceArgs `json:"excludeProjectVariableRegex" jsonschema:"Exclude a project variable from being exported based on regex match. WARNING: steps that used this variable may no longer function correctly."`
	ExcludeVariableEnvironmentScopes StringSliceArgs `json:"excludeVariableEnvironmentScopes" jsonschema:"Exclude an environment when it appears in a variable's environment scope. WARNING: variables scoped to this environment will no longer have that environment scope applied."`

	ExcludeAllTenantVariables    bool            `json:"excludeAllTenantVariables" jsonschema:"Exclude all tenant variables from being exported. WARNING: steps that used this variable may no longer function correctly."`
	ExcludeTenantVariables       StringSliceArgs `json:"excludeTenantVariables" jsonschema:"Exclude a tenant variable from being exported. WARNING: steps that used this variable may no longer function correctly."`
	ExcludeTenantVariablesExcept StringSliceArgs `json:"excludeTenantVariablesExcept" jsonschema:"All tenant variables except those defined with excludeTenantVariablesExcept are excluded. WARNING: steps that used other variables may no longer function correctly."`
	ExcludeTenantVariablesRegex  StringSliceArgs `json:"excludeTenantVariablesRegex" jsonschema:"Exclude a tenant variable from being exported based on regex match. WARNING: steps that used this variable may no longer function correctly."`

	ExcludeAllSteps    bool            `json:"excludeAllSteps" jsonschema:"Exclude all steps when exporting projects or runbooks. WARNING: variables scoped to this step will no longer have the step scope applied."`
	ExcludeSteps       StringSliceArgs `json:"excludeSteps" jsonschema:"A step to be excluded when exporting projects or runbooks. WARNING: variables scoped to this step will no longer have the step scope applied."`
	ExcludeStepsRegex  StringSliceArgs `json:"excludeStepsRegex" jsonschema:"A step to be excluded when exporting projects or runbooks based on regex match. WARNING: variables scoped to this step will no longer have the step scope applied."`
	ExcludeStepsExcept StringSliceArgs `json:"excludeStepsExcept" jsonschema:"All steps except those defined with excludeStepsExcept are excluded when exporting a project or runbook. WARNING: variables scoped to other steps will no longer have the step scope applied."`

	ExcludeAllChannels     bool            `json:"excludeAllChannels" jsonschema:"Exclude all channels from being exported. WARNING: Variables and steps that were scoped to channels will become unscoped."`
	ExcludeChannels        StringSliceArgs `json:"excludeChannels" jsonschema:"Exclude a channel from being exported. WARNING: Variables and steps that were scoped to channels will become unscoped."`
	ExcludeChannelsRegex   StringSliceArgs `json:"excludeChannelsRegex" jsonschema:"Exclude a channel from being exported based on regex match. WARNING: Variables and steps that were scoped to channels will become unscoped."`
	ExcludeChannelsExcept  StringSliceArgs `json:"excludeChannelsExcept" jsonschema:"All channels except those defined with excludeChannelsExcept are excluded. WARNING: Variables and steps that were scoped to channels will become unscoped."`
	ExcludeInvalidChannels bool            `json:"excludeInvalidChannels" jsonschema:"Channels that reference packages that are no longer defined in the deployment process will be excluded. WARNING: Variables and steps that were scoped to channels will become unscoped."`

	ExcludeAllRunbooks    bool            `json:"excludeAllRunbooks" jsonschema:"Exclude all runbooks when exporting a project or space. WARNING: variables scoped to this runbook will no longer have the runbook scope applied."`
	ExcludeRunbooks       StringSliceArgs `json:"excludeRunbook" jsonschema:"A runbook to be excluded when exporting a single project. WARNING: variables scoped to this runbook will no longer have the runbook scope applied."`
	ExcludeRunbooksRegex  StringSliceArgs `json:"excludeRunbookRegex" jsonschema:"A runbook to be excluded when exporting a single project based on regex match. WARNING: variables scoped to this runbook will no longer have the runbook scope applied."`
	ExcludeRunbooksExcept StringSliceArgs `json:"excludeRunbooksExcept" jsonschema:"All runbooks except those defined with excludeRunbooksExcept are excluded when exporting a single project. WARNING: variables scoped to other runbooks will no longer have the runbook scope applied."`
	...
	ExcludeTriggersExcept StringSliceArgs `json:"excludeTriggersExcept" jsonschema:"All triggers except those defined with excludeTriggersExcept are excluded when exporting a single project."`

	ExcludeLibraryVariableSets       StringSliceArgs `json:"excludeLibraryVariableSet" jsonschema:"A library variable set to be excluded when exporting a single project. WARNING: projects that linked this library variable set will no longer include these variables."`
	ExcludeLibraryVariableSetsRegex  StringSliceArgs `json:"excludeLibraryVariableSetRegex" jsonschema:"A library variable set to be excluded when exporting a single project based on regex match. WARNING: projects that linked this library variable set will no longer include these variables."`
	ExcludeLibraryVariableSetsExcept StringSliceArgs `json:"excludeLibraryVariableSetsExcept" jsonschema:"All library variable sets except those defined with excludeLibraryVariableSetsExcept are excluded. WARNING: projects that linked other library variable sets will no longer include these variables."`
	ExcludeAllLibraryVariableSets    bool            `json:"excludeAllLibraryVariableSets" jsonschema:"Exclude all library variable sets. WARNING: projects that linked this library variable set will no longer include these variables."`

	ExcludeEnvironments       StringSliceArgs `json:"excludeEnvironments" jsonschema:"An environment to be excluded when exporting a single project. WARNING: this can have unexpected side effects, such as variables becoming unscoped. The exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeEnvironmentsRegex  StringSliceArgs `json:"excludeEnvironmentsRegex" jsonschema:"An environment to be excluded when exporting a single project based on regex match. WARNING: this can have unexpected side effects, such as variables becoming unscoped. The exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeEnvironmentsExcept StringSliceArgs `json:"excludeEnvironmentsExcept" jsonschema:"All environments except those defined with excludeEnvironmentsExcept are excluded. WARNING: this can have unexpected side effects, such as variables becoming unscoped. The exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllEnvironments    bool            `json:"excludeAllEnvironments" jsonschema:"Exclude all environments. WARNING: this can have unexpected side effects, such as variables becoming unscoped. The exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeFeeds       StringSliceArgs `json:"excludeFeeds" jsonschema:"A feed to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeFeedsRegex  StringSliceArgs `json:"excludeFeedsRegex" jsonschema:"A feed to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeFeedsExcept StringSliceArgs `json:"excludeFeedsExcept" jsonschema:"All feeds except those defined with excludeFeedsExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllFeeds    bool            `json:"excludeAllFeeds" jsonschema:"Exclude all feeds. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeProjectGroups       StringSliceArgs `json:"excludeProjectGroups" jsonschema:"A project group to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeProjectGroupsRegex  StringSliceArgs `json:"excludeProjectGroupsRegex" jsonschema:"A project group to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeProjectGroupsExcept StringSliceArgs `json:"excludeProjectGroupsExcept" jsonschema:"All project groups except those defined with excludeProjectGroupsExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllProjectGroups    bool            `json:"excludeAllProjectGroups" jsonschema:"Exclude all project groups. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeAccounts       StringSliceArgs `json:"excludeAccounts" jsonschema:"An account to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAccountsRegex  StringSliceArgs `json:"excludeAccountsRegex" jsonschema:"An account to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAccountsExcept StringSliceArgs `json:"excludeAccountsExcept" jsonschema:"All accounts except those defined with excludeAccountsExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllAccounts    bool            `json:"excludeAllAccounts" jsonschema:"Exclude all accounts. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeCertificates       StringSliceArgs `json:"excludeCertificates" jsonschema:"A certificate to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeCertificatesRegex  StringSliceArgs `json:"excludeCertificatesRegex" jsonschema:"A certificate to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeCertificatesExcept StringSliceArgs `json:"excludeCertificatesExcept" jsonschema:"All certificates except those defined with excludeCertificatesExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllCertificates    bool            `json:"excludeAllCertificates" jsonschema:"Exclude all certificates. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeLifecycles       StringSliceArgs `json:"excludeLifecycles" jsonschema:"A lifecycle to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeLifecyclesRegex  StringSliceArgs `json:"excludeLifecyclesRegex" jsonschema:"A lifecycle to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeLifecyclesExcept StringSliceArgs `json:"excludeLifecyclesExcept" jsonschema:"All lifecycles except those defined with excludeLifecyclesExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllLifecycles    bool            `json:"excludeAllLifecycles" jsonschema:"Exclude all lifecycles. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeWorkerpools       StringSliceArgs `json:"excludeWorkerPools" jsonschema:"A worker pool to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeWorkerpoolsRegex  StringSliceArgs `json:"excludeWorkerPoolsRegex" jsonschema:"A worker pool to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeWorkerpoolsExcept StringSliceArgs `json:"excludeWorkerPoolsExcept" jsonschema:"All worker pools except those defined with excludeWorkerPoolsExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllWorkerpools    bool            `json:"excludeAllWorkerPools" jsonschema:"Exclude all worker pools. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`

	ExcludeMachinePolicies       StringSliceArgs `json:"excludeMachinePolicies" jsonschema:"A machine policy to be excluded when exporting a single project. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeMachinePoliciesRegex  StringSliceArgs `json:"excludeMachinePoliciesRegex" jsonschema:"A machine policy to be excluded when exporting a single project based on regex match. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeMachinePoliciesExcept StringSliceArgs `json:"excludeMachinePoliciesExcept" jsonschema:"All machine policies except those defined with excludeMachinePoliciesExcept are excluded. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	ExcludeAllMachinePolicies    bool            `json:"excludeAllMachinePolicies" jsonschema:"Exclude all machine policies. WARNING: the exported module is unlikely to be complete and will fail to apply if this option is enabled."`
	...
	ExcludeMachineProxiesRegex  StringSliceArgs `json:"excludeMachineProxiesRegex" jsonschema:"A machine proxy to be excluded when exporting a single project based on regex match."`
	ExcludeMachineProxiesExcept StringSliceArgs `json:"excludeMachineProxiesExcept" jsonschema:"All machine proxies except those defined with excludeMachineProxiesExcept are excluded."`
	ExcludeAllMachineProxies    bool            `json:"excludeAllMachineProxies" jsonschema:"Exclude all machine proxies."`

	ExcludeTenantTags StringSliceArgs `json:"excludeTenantTags" jsonschema:"Exclude an individual tenant tag from being exported. Tags are in the format 'taggroup/tagname'. WARNING: Steps that were set to run on tenants with excluded tags will no longer have that condition applied."`

	ExcludeTenantTagSets       StringSliceArgs `json:"excludeTenantTagSets" jsonschema:"Exclude a tenant tag set from being exported. WARNING: Steps that were set to run on tenants with excluded tags will no longer have that condition applied."`
	ExcludeTenantTagSetsRegex  StringSliceArgs `json:"excludeTenantTagSetsRegex" jsonschema:"Exclude tenant tag sets from being exported based on a regex. WARNING: Resources scoped to tenant tag sets will be unscoped."`
	ExcludeTenantTagSetsExcept StringSliceArgs `json:"excludeTenantTagSetsExcept" jsonschema:"Exclude all tenant tag sets except for those defined in this list. WARNING: Resources scoped to tenant tag sets will be unscoped."`
	ExcludeAllTenantTagSets    bool            `json:"excludeAllTenantTagSets" jsonschema:"Exclude all tenant tag sets from being exported. WARNING: Resources scoped to tenant tag sets will be unscoped."`
	...
	ExcludeTenantsWithTags StringSliceArgs `json:"excludeTenantsWithTag" jsonschema:"Exclude any tenant with this tag from being exported. This is useful when using tags to separate tenants that can be exported with those that should not. Tags are in the format TagGroupName/TagName."`
	ExcludeTenantsExcept   StringSliceArgs `json:"excludeTenantsExcept" jsonschema:"Exclude all tenants except for those defined in this list. The tenants in excludeTenants take precedence, so a tenant defined here and in excludeTenants is excluded."`
	...
	ExcludeProjectsExcept StringSliceArgs `json:"excludeProjectsExcept" jsonschema:"All projects except those defined with excludeProjectsExcept are excluded. This is only used when exporting a space."`
	ExcludeProjectsRegex  StringSliceArgs `json:"excludeProjectsRegex" jsonschema:"Exclude a project from being exported based on regex match. This is only used when exporting a space."`
	...
	ExcludeAllTargets                bool            `json:"excludeAllTargets" jsonschema:"Exclude all targets from being exported. WARNING: Variables that were scoped to targets will become unscoped."`
	ExcludeTargets                   StringSliceArgs `json:"excludeTargets" jsonschema:"Exclude targets from being exported. WARNING: Variables that were scoped to targets will become unscoped."`
	ExcludeTargetsRegex              StringSliceArgs `json:"excludeTargetsRegex" jsonschema:"Exclude targets from being exported based on a regex. WARNING: Variables that were scoped to targets will become unscoped."`
	ExcludeTargetsExcept             StringSliceArgs `json:"excludeTargetsExcept" jsonschema:"Exclude all targets except for those defined in this list. The targets in excludeTargets take precedence, so a target defined here and in excludeTargets is excluded. WARNING: Variables that were scoped to other targets will become unscoped."`
	ExcludeTargetsWithNoEnvironments bool            `json:"excludeTargetsWithNoEnvironments" jsonschema:"Exclude targets that have had all their environments excluded. WARNING: Variables that were scoped to targets will become unscoped."`
	...
	ExcludeWorkersExcept StringSliceArgs `json:"excludeWorkersExcept" jsonschema:"Exclude all workers except for those defined in this list. The targets in excludeWorkers take precedence, so a worker defined here and in excludeWorkers is excluded."`
	...
	ExcludeDeploymentFreezesExcept StringSliceArgs `json:"excludeDeploymentFreezesExcept" jsonschema:"All deployment freezes except those defined with excludeDeploymentFreezesExcept are excluded."`
