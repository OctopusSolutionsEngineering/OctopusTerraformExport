...
// @description 25/01/2023, 7:12:06 am
// ==/UserScript==
...
function dumpToHcl() {
    try {
        if (window.location.href.split("/")[5] === "projects") {
            ...
            // Add a nav menu item
            let nav = getElementByXpath("(//main//nav)[2]")
                ...
                onload: function(e) {
                    ...
                    WebAssembly.instantiate(e.response, go.importObject)
                        .then((result) => {
                            ...
                            convertProject(server, space, project)
                                .then(function(hcl) {

                                    hclOutput.style.cursor = 'pointer';

                                    hclOutput.addEventListener('mouseover', function handleMouseOver() {
                                        hclOutput.style.color = '#1A77CA';
                                    });
...
// Wait for the nav menu to be displayed before we start dumping the HCL
waitForKeyElements ("a:contains('Deployments')", dumpToHcl);
